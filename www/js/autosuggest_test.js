...
function Models(url) {
    ...
    */
    ...
    this.keywords = new PouchDB("keywords2");

/*
    var remote_keywords2 = new PouchDB(couchurl+"/keywords2");
    remote_keywords2.replicate.to(this.keywords, function(err, result) {
        if (err) {
            console.log(err);
        }
    });
...
/*
    this.keywords2 = new PouchDB("keywords2");
    var remote_keywords2 = new PouchDB(couchurl+"/keywords2");

    console.log("replicate.to keywords2");
    this.keywords.replicate.to(this.keywords2, function(err, result) {
        if (err) {
            console.log(err);
        }
    });
    this.keywords.replicate.to(remote_keywords2, function(err, result) {
        if (err) {
            console.log(err);
        }
    });
*/
    ...
    this.sentences = new PouchDB("sentences2");

/*
    var remote_sentences2 = new PouchDB(couchurl+"/sentences2");
    remote_sentences2.replicate.to(this.sentences, function(err, result) {
        if (err) {
            console.log(err);
        }
    });
...
/*
    this.sentences2 = new PouchDB("sentences2");
    var remote_sentences2 = new PouchDB(couchurl+"/sentences2");

    console.log("replicate.to sentences2");
    this.sentences.replicate.to(this.sentences2, function(err, result) {
        if (err) {
            console.log(err);
        }
    });
    this.sentences.replicate.to(remote_sentences2, function(err, result) {
        if (err) {
            console.log(err);
        }
    });
*/
    ...
    this.similar_keywords = new PouchDB("similar_keywords2");

/*
    var remote_similar_keywords2 = new PouchDB(couchurl+"/similar_keywords2");
    remote_similar_keywords2.replicate.to(this.similar_keywords, function(err, result) {
        if (err) {
            console.log(err);
        }
    });
...
/*
    this.similar_keywords2 = new PouchDB("similar_keywords2");
    var remote_similar_keywords2 = new PouchDB(couchurl+"/similar_keywords2");

    console.log("replicate.to similar_keywords2");
    this.similar_keywords.replicate.to(this.similar_keywords2, function(err, result) {
        if (err) {
            console.log(err);
        }
    });
    this.similar_keywords.replicate.to(remote_similar_keywords2, function(err, result) {
        if (err) {
            console.log(err);
        }
    });
*/
    ...
    if (!JSON.parse(localStorage.getItem('isDbDownloaded2'))) 
        localStorage.setItem('isDbDownloaded2', JSON.stringify(false));
    
    this.initialized = JSON.parse(localStorage.getItem('isDbDownloaded2'));
...
Models.prototype = {
    ...
    initializePersistentQueries: function(proceedCb) { 
        ...
        var _this = this;
        var isInitialized = JSON.parse(localStorage.getItem('isDbDownloaded2'));
        ...
        _this.keywords.info().then(function(kw_result){
            _this.similar_keywords.info().then(function(sk_result){
                _this.sentences.info().then(function(s_result){
                    ...
                    if((kw_result.doc_count+sk_result.doc_count+s_result.doc_count) == 0 || !JSON.parse(localStorage.getItem('isDbDownloaded2'))){
                        callback();
                    }
                });
            });
    ...
    start: function(downloadingCb,proceedCb){
        ...
        this.ifEmptyData(function(){
            ...
            downloadingCb();
            initialize(function(){
               localStorage.setItem('isDbDownloaded2', true);
...
View.prototype = {
    ...
    appendTableData: function(results, isHistory, isSuggestion) {
	    ...
	    console.time("getSentences");
            _this.getSentences(ids).then(function(sentences){
	        console.timeEnd("getSentences");
    ...
    getSimilarKeywords: function(ids) {
        var _this = this;
	console.time("showSimilarKeywords");
        _this._models.showSimilarKeywords(ids).then(function(keywordIdB){
	    console.timeEnd("showSimilarKeywords");
